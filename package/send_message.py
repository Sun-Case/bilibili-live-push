...
class Message:
    ...
    def __init__(self, q: asyncio.Queue, loop, config: dict, room_list: list, live_template: str, preparing_template: str, echoQ) -> None:
        self.q = q
        self.loop = loop
        self.config = config
        self.room_status = {}
        ...
        while True:
            ...
            if data["status"] != self.room_status[data["room_id"]]:
                ...
                content = self.__content__(
                    data["name"], data["status"], data["room_id"], data["true_room"], data["uid"])
                for k, v in self.config.items():
                    if k == "Telegram" and v["status"]:
                        self.loop.create_task(
                            self.Telegram(content, data["room_id"]))
                    if k == "ServerChan" and v["status"]:
                        self.loop.create_task(
                            self.ServerChan(content, data["room_id"]))

    def __content__(self, name: str, status: bool, room_id: int, true_room: int, uid: int):
        if status:
    ...
    async def Telegram(self, content: str, room_id):
        url = "https://api.telegram.org/bot%s/sendMessage" % self.config["Telegram"]["bot_token"]
        data = {"chat_id": self.config["Telegram"]["user_id"], "text": content}
        async with aiohttp.ClientSession() as session:
            for a in range(0, 3):
                try:
                    await session.post(url, data=data, timeout=3)
                    await self.echoQ.put({"code": 0, "data": "Telegram 发送成功 %s" % (a+1), "id": room_id})
                    break
                except:
                    await self.echoQ.put({"code": 0, "data": "Telegram 发送失败 %s" % (a+1), "id": room_id})
