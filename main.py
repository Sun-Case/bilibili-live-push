...
preparing_template = "【${name}】 room_id: ${room_id}, true_id: ${true_room}, uid: ${uid} 下播啦"
...
if __name__ == '__main__':
    ...
    print("")
    echo.echo([["#", "直播间ID"]]+[["%s" % (i+1), "%s" % v]
              for i, v in enumerate(roomid_list)])
    print("")
    ...
    tasks.append(asyncio.ensure_future(send_message.Message(
        q, loop, config, roomid_list, live_template, preparing_template, echoQ).run()))
